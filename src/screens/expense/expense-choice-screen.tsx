...
export default function ExpenseChoiceScreen() {
  ...
  const { participants, groupId } = useLocalSearchParams<{
    participants: string;
    groupId?: string;
  }>();
  ...
  const processExtraction = async (imageUri: string) => {
    try {
      ...
      const { total_amount } = await expensesApi.extractReceiptTotal({
        s3_key: object_key,
      });
      ...
      Alert.alert(
        "Extraction Error",
        error?.message || "Could not extract total from the receipt.",
      );
    } finally {
      setLoading(false);
      setLoadingStep("");
  ...
  const handleExtractChoice = () => {
    if (isBusy) return;
    Alert.alert("Extract Total", "Choose a source for your receipt", [
      {
        text: "Camera",
        onPress: async () => {
    ...
    router.push({
      ...
      params: {
        ...
        participantIds: JSON.stringify(
          parsedParticipants.map((p: any) => p.id),
        ),
      },
    });
  };
        ...
        >
          <Card style={[styles.methodCard, styles.aiCard]}>
            ...
            <View style={styles.cardContent}>
              <Typography.SubHeader style={styles.cardTitleWhite}>
                Split with AI
              </Typography.SubHeader>
              <Typography.Body style={styles.cardDescWhite}>
                Upload receipt & tell AI how to divide it among everyone.
              </Typography.Body>
            </View>
          </Card>
        </TouchableOpacity>
        ...
        >
          <Card style={styles.methodCard}>
            ...
            <View style={styles.cardContent}>
              ...
              <Typography.Body style={styles.cardDesc}>
                Photo to automatically extract the total amount.
              </Typography.Body>
            </View>
          </Card>
        </TouchableOpacity>
        ...
        >
          <Card style={styles.methodCard}>
            ...
            <View style={styles.cardContent}>
              ...
              <Typography.Body style={styles.cardDesc}>
                Enter details and amount yourself.
              </Typography.Body>
            </View>
          </Card>
        </TouchableOpacity>
